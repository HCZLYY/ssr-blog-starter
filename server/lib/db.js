...
// --- 强制先加载 server/.env，保证任何加载 db.js 的时刻都有正确 env ----------
const dotenv = require('dotenv');
const envPath = path.resolve(__dirname, '..', '.env'); // server/.env
dotenv.config({ path: envPath });
// -------------------------------------------------------------------------------

// accept either DATABASE_* or DB_* names (prefer DATABASE_ because your .env uses that)
const DB_HOST = process.env.DATABASE_HOST || process.env.DB_HOST || '127.0.0.1';
const DB_PORT = Number(process.env.DATABASE_PORT || process.env.DB_PORT || 6606);
const DB_USER = process.env.DATABASE_USER || process.env.DB_USER || 'ssr_user';
const DB_PASSWORD = process.env.DATABASE_PASSWORD || process.env.DB_PASSWORD || 'password';
const DB_NAME = process.env.DATABASE_NAME || process.env.DB_NAME || 'ssr_blog';
...
console.log('[lib/db] final DB config ->', {
    host: DB_HOST,
    port: DB_PORT,
    user: DB_USER,
    database: DB_NAME
...
module.exports.query = module.exports.execute;    // 让 renderPage 可以用 db.query()